...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* vertical edge label */}
      <div className="pointer-events-none absolute right-3 top-1/2 z-10 hidden -translate-y-1/2 xl:block">
        <span
          className="t-eyebrow whitespace-nowrap text-muted"
          style={{ writingMode: "vertical-rl" }}
        >
          AARNA ENERGIES · ROOFTOP SOLAR
        </span>
      </div>

      <motion.div
        style={{ y: yHead, opacity: fade }}
        className="section-x relative z-10 flex flex-1 flex-col justify-center pt-28 pb-16"
      >
        {/* eyebrow */}
        <motion.div
      ...
      >
        <dl className="grid grid-cols-2 gap-x-6 gap-y-6 md:grid-cols-4">
          {hero.stats.map((s, i) => (
            ...
            >
              <dt className="flex items-baseline gap-0.5 text-3xl font-semibold tracking-tight text-cream md:text-[2.5rem]">
                {s.value}
                <span className="text-sienna">{s.suffix}</span>
              </dt>
              <dd className="mt-1.5 text-[0.82rem] font-medium leading-snug text-muted">
                {s.label}
              </dd>
            </motion.div>
          ))}
